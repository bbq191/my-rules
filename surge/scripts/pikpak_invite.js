...
$argument = '67814749'
*/
const http = (t, e = "post") = >new Promise((i, $) = >{
    $httpClient[e](t, (e, a, r) = >{
        if (a ? .status === 200) i(JSON.parse(r));
        else {
            let n = r.match(/[\u4e00-\u9fff]+/g) ? .join(" ");
            $(`err: $ {
                e || n
            }`)
        }
...
class Pikpak {
    constructor(t) {
        this.captcha_token = t[2],
        this.timestamp = t[3],
        this.captcha_sign = t[4],
        this.body = {
            client_id: t[0],
            device_id: t[1]
        },
    ...
    async getEmailCode() {
        ...
        e = await this.sendVerificationRequest(t),
        i = await this.getVerificationCode(),
        $ = await this.getVerificationToken(e, i);
        await this.gregister($, i, t);
        let a = await this.getCaptchaToken("POST:/v1/auth/signin", t),
        r = await this.signin(a),
        n = await this.getCaptchaToken("POST:/vip/v1/activity/invite", a);
